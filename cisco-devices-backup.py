...
# file should contain a list of devices in format: ip,username,password,device_type
DEVICE_FILE_PATH = '/home/alabarym/git/alabarym/2020.04-devnet/inventory.csv'
# complete path to backup directory
BACKUP_DIR_PATH = '/home/alabarym/git/alabarym/2020.04-devnet'
...
def get_previous_backup_file_path(hostname, curent_backup_file_path):
    ...
    for file_name in os.listdir(os.path.join(BACKUP_DIR_PATH, hostname)):
        ...
        if file_name.endswith('.txt') and file_name != current_backup_filename:
            ...
            filename_datetime = datetime.datetime.strptime(
                file_name.strip('.txt')[len(hostname)+1:], '%Y_%m_%d-%H_%M_%S')
    ...
    if len(backup_files) > 0:
        ...
        previous_backup_file_path = os.path.join(
            BACKUP_DIR_PATH, hostname, backup_files[previous_backup_key])
...
def process_target(device, timestamp):
    ...
    backup_result = create_backup(
        connection, backup_file_path, device['hostname'])
...
def get_backup_file_path(hostname, timestamp):
    ...
    backup_file_path = os.path.join(
        BACKUP_DIR_PATH, hostname, '{}-{}.txt'.format(hostname, timestamp))
    ...
    with mp.Pool(4) as pool:
        ...
        for device in device_list:
            processes.append(pool.apply_async(
                process_target, args=(device, timestamp)))
        # Waiting for results...
